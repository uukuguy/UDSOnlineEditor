...
应用程序向导已为您创建了此 base 项目，您可以在此基础之上
编写自己的动态链接库 (DLL)。
...
base.idl
    ...
    和 coclass 的 IDL 定义。
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明                 (base.h)
        GUID 定义                                (base_i.c)
        一个类型库                               (base.tlb)
        封送代码                                 (base_p.c 和 dlldata.c)

base.h
    此文件包含在 base.idl 中定义的项的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 base.pch 的预编译头 (PCH) 文件
...
baseps.vcproj
	...
	dlldata.c、base_i.c 和 base_p.c，它们。
