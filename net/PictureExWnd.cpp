...
BOOL CPictureExWnd::Load(LPCTSTR szFileName)
{
	...
	//if (!(file = _tfopen_s(szFileName,_T("rb"))))
...
BOOL CPictureExWnd::Load(LPCTSTR szResourceName, LPCTSTR szResourceType)
{
	...
	HRSRC hPicture = FindResource(_AtlBaseModule.GetResourceInstance(),szResourceName,szResourceType);
	HGLOBAL hResData;
	if (!hPicture || !(hResData = LoadResource(_AtlBaseModule.GetResourceInstance(),hPicture)))
	{
		ATLTRACE(_T("Load (resource): Error loading resource %s\n"),szResourceName);
		return FALSE;
	};
	DWORD dwSize = SizeofResource(_AtlBaseModule.GetResourceInstance(),hPicture);
